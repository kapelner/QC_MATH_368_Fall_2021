...
\date{Due by email 11:59PM Thursday, September 30, \the\year{} \\ \vspace{0.5cm} \small (this document last updated \today ~at \currenttime)}
}
...
\hardsubproblem{Let $X \sim \geometric{p_x}$ independent of $Y \sim \geometric{p_y}$. Find $\prob{X > Y}$ using the method we did in class. Note that $p_x$ and $p_y$ are now different.}\spc{10}
...
\intermediatesubproblem{[OPTIONAL] Prove the combinatorial identity in (b).}\spc{9}
...
Draw one marble with replacement 37 times. What is the probability of getting 10 red, 17 green, 6 blue and 4 yellow? Compute explicitly to the nearest two significant digits.}\spc{7}
...
\extracreditsubproblem{[MA] If $\X \sim \multinomial{n}{\p}$, find a proof for $\cov{X_i}{X_j} = -np_i p_j$ that is qualitatively different than the one we did in class.}\spc{8}

\intermediatesubproblem{If $\X \sim \multinomial{n}{\p}$ where $\dime{\X} = K$ and $\p = \oneover{K}\onevec_K$. What is the limit of  $\cov{X_i}{X_j}$ as $K$ gets large but $n$ is fixed. Why does this make sense?}\spc{3}
...
where $\sd{\cdot}$ denotes the standard deviation of a r.v., the square root of its variance. Find $\corr{X_i,\,X_j}$ for two arbitrary elements in the r.v. vector $\X \sim \multinomial{n}{\p}$. }\spc{4}



\easysubproblem{If $\c = \bracks{1 ~2 ~3 ~4}^\top$, compute the inner product $\c^\top \c$ and the outer product $\c \c^\top$.}\spc{3}
...
\easysubproblem{Resolve as best as possible: $\int_{\reals} \indic{x \in \bracks{0, c}}\mathrm{d}x$ where $c \in \reals$.}\spc{1.5}

\easysubproblem{Resolve as best as possible: $\int_{-\infty}^d \indic{x \in \bracks{0, c}}\mathrm{d}x$ where $c,d \in \reals$.}\spc{1.5}

\easysubproblem{Resolve as best as possible: $\int_{d}^{d+1} \indic{x \in \bracks{0, c}}\mathrm{d}x$ where $c,d \in \reals$.}\spc{1.5}

\intermediatesubproblem{Resolve as best as possible: $\int_{d}^{d+1} \indic{x \in \bracks{c, c + 1}}\mathrm{d}x$ where $c,d \in \reals$.}\spc{1.5}
