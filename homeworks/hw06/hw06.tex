...
Reading is still \textit{required}. For this homework set, read on your own about the different inequalities we discussed some of which were tail bounds, convergence in distribution, convergence in probability to a constant and convergence in law to a constant, iterated expectation and law of total variance.
...
\begin{enumerate}
\easysubproblem{Provide the definitions of $\convd$, $\convp$ and $\convLp{r}$ below.}
\inred{You do not need a definition of $\convLp{r}$}
\spc{7}
...
\intermediatesubproblem{Show that $X_n \convLp{r} 0$ for all $r \geq 1$.}
\inred{Skip this question}\spc{7}
...
\intermediatesubproblem{If $X_n \sim \exponential{n}$, show that $X_n \convLp{1} 0$.}\inred{Skip this question}\spc{7}
